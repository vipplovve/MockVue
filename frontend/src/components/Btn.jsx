...
const Btn = ({ content, onClick, className = '', ...props }) => {
  return (
    <button
      onClick={onClick}
      className={`relative inline-flex items-center justify-center px-8 py-2.5 overflow-hidden tracking-tighter text-white bg-gray-600 rounded-md group ${className}`}
      {...props}
    >
      ...
      <span className="relative text-base font-semibold">{content}</span>
    </button>
  )
}
